...
|---|---|---|---|
| 1 | [Production Orders](../10-production/production-orders.md) | Search for the existing WO or use the visible `New Production Orders` button if the demo script asks you to create one. | The row shows the expected part, quantity, date, and status. |
...
| 4 | [Planning](../10-production/planning.md) | Schedule or adjust only when the screen shows a clear scheduling action for the demo. | The work remains visible after refresh and still matches the WO. |
...
| Open [OEE](../10-production/dashboards.md) | [Production Dashboards](../10-production/dashboards.md) | Queue verification is complete or the demo asks for a high-level trend check. | Open `Production / OEE` from the sidebar. | OEE page is open, but this manual version treats dashboards as open-and-observe only. | OEE URL/page screenshot and note that metric definitions remain owner-confirmed, not manual-confirmed. |
...
| [OEE](../10-production/dashboards.md) page is blank or unclear | Demo owner | Dashboard URL, filter period, and screenshot request |
