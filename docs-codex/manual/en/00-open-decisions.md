...
Use this page before running a customer demo or first-user walkthrough. It records the items that still need visible evidence or owner confirmation before the manual can give exact click-by-click instructions.
...
|---|---|---|
| Open | The manual cannot safely name the exact click, field, metric, or setup relationship yet. | Keep the instruction conservative and route the user to the evidence request. |
...
| Queue dispatch and row actions | [Operator Walkthrough](01-workflows/operator-run-next-job.md), [Queue System](10-production/queue-system.md), [Supervisor Triage](01-workflows/supervisor-triage.md) | Choose a job only by confirmed dispatch rule, assigned work area, or supervisor instruction. Do not use row order alone. | Queue screenshot with filters, WO/job, part/process, status, row action label, and result after open/start. | Production supervisor |
...
| Language and translation setup | [Admin Setup Checklist](01-workflows/admin-setup-checklist.md), [Start Here](00-start-here.md) | The manual is bilingual, but the app language setup process remains unconfirmed. | Confirmed sidebar path for language/translation setup, visible fields, save action, and expected language behavior. | Implementation owner |
| Quality and Inspection Planning demo scope | [Quality Engineer Manual](03-by-role/quality-engineer.md), [Inspection Planning](30-quality/inspection-planning.md), [Inspection Records](30-quality/inspection-records.md) | Keep Quality/QC and Inspection Planning visible, but confirm scope before treating them as required customer-demo steps. | Demo-scope decision, required inspection example, saved record, and NCR/calibration handling where relevant. | Quality owner or demo owner |
