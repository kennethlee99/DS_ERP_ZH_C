...
在计划员、操作员或主管演示前，请使用本检查清单。本页区分手册可以确认的内容，以及仍需要演示负责人澄清的内容。
...
| 顺序  | 区域    | 检查                           | 页面                                                                                                                                   |
| --- | ----- | ---------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ |
| 1   | 用户访问  | 演示用户可以登录，并具有预期角色。            | [用户和角色](../40-administration/users-and-roles.md)                                                                                     |
| 2   | 工人身份  | 确认演示使用工人记录、用户账号，还是两者都用。      | `待决`                                                                                                                                 |
...
| 9   | 语言标签  | 可见语言和翻译页面需演示负责人确认后，才能写成设置步骤。 | `待决`                                                                                                                                 |

## 安全快速开始

当您只需要让演示用户可以复查，而不是重新设计访问权限时，使用本表。

| 步骤 | 操作 | 遇到以下情况就停止 |
|---|---|---|
| 1 | 在[用户和角色](../40-administration/users-and-roles.md)中确认登录账号存在且可用。 | 用户缺失或不可用。 |
| 2 | 确认用户拥有演示场景预期角色。 | 角色缺失，或侧边栏与场景不匹配。 |
| 3 | 确认操作员场景是否需要独立于登录账号的工人身份。 | 应用显示不清楚的工人或工作区域字段。 |
| 4 | 登录后确认预期菜单项和可见动作出现。 | 菜单或动作缺失。 |
| 5 | 计划流程前确认[零件](../20-engineering/parts.md)、[BOM](../20-engineering/bom.md)、[配方](../20-engineering/recipes.md)、[机台](../20-engineering/machines.md)、[NC 程序](../20-engineering/nc-programs.md)和检验设置可见。 | 演示物料的任何前提页面缺失或为空。 |

## 演示准备矩阵

在计划员、操作员或主管开始冷启动演示前填写本矩阵。请使用应用中实际可见的标签。未确认的值保留为 `needs-decision`，并按证据列要求补充截图。

| 演示角色 | 登录用户 | 工人身份 | 角色名称 | 必需侧边栏页面 | 必需可见动作 | 权限标签原文 | 语言预期 | 必需主数据 | 保留证据 |
|---|---|---|---|---|---|---|---|---|---|
...
当前手册无法证明 Users 和 Workers 是同一记录、关联记录还是分开设置。操作员演示前必须把这点作为实施决定。请记录演示用户登录账号、屏幕上显示的工人身份、机台/工作区域和角色。

## 角色和权限

使用[用户和角色](../40-administration/users-and-roles.md)确认可见访问。除非管理员或演示负责人确认含义，否则不要自行解释看起来像技术标签的权限文字。如果页面或操作缺失，请记录用户、角色、预期页面/操作和当前截图。

| 术语 | 实际含义 |
|---|---|
| 用户 | 用于登录网页应用的账号。 |
| 工人 | 如果演示使用，则表示车间身份或分派记录；在确认前，用户与工人的关系仍是待决事项。 |
| 角色 | 分配给用户的可见访问分组。 |
...
手册本身是双语的，但可见语言和翻译管理页面仍是待决事项。在演示负责人确认预期语言流程前，不要声称已有完整翻译设置流程。
...
在本页成为完整实施指南前，还需要用户与工人关系、带负责人解释的角色权限标签、语言/翻译设置，以及显示预期侧边栏的已确认演示用户登录截图。这些事项记录在[证据与待决事项登记](../00-open-decisions.md)中。
