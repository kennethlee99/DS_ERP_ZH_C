...
1. **开始条件：** 您以计划员身份登录，准备释放第一张样板 WO 或培训 WO。
   **打开屏幕：** [管理员设置清单](../01-workflows/admin-setup-checklist.md)。
   **步骤：** 确认计划员角色可打开工单、计划、队列系统和看板。
   **预期结果：** 该角色可见必需计划页面。
   **停止/联系条件：** 如果角色或权限标签为 `待确认`，请停止并联系管理员或计划负责人。
2. **开始条件：** 计划员访问权限可见。
   **打开屏幕：** [零件](../20-engineering/parts.md)和 [BOM](../20-engineering/bom.md)。
   **步骤：** 确认零件修订和 BOM 结构与培训 WO 一致。
   **预期结果：** 该物料可用于第一天 WO 证据记录。
   **停止/联系条件：** 如果零件或结构缺失，请停止并联系生产工程或计划负责人。
3. **开始条件：** 物料准备已确认。
   **打开屏幕：** [配方](../20-engineering/recipes.md)、[机台](../20-engineering/machines.md)和 [NC 程序](../20-engineering/nc-programs.md)。
   **步骤：** 确认所选作业的配方、机台/工作区域和 NC 程序准备情况。
   **预期结果：** 路线可见，足以支持计划复查。
   **停止/联系条件：** 如果路线、机台能力或 NC 程序准备情况不明确，请停止并联系生产工程。
4. **开始条件：** 主数据和路线准备可见。
   **打开屏幕：** [计划员冷启动流程](../01-workflows/planner-cold-start.md)。
   **步骤：** 只用已确认控制搜索或建立指定 WO，并记录可见状态和操作证据。
   **预期结果：** 不猜测释放、排程或状态控制，也能复查 WO。
   **停止/联系条件：** 如果释放、排程或状态操作没有标签，请停止并联系计划负责人或负责人。
5. **开始条件：** WO 预期已排程/释放，可进入队列复查。
   **打开屏幕：** [队列系统](../10-production/queue-system.md)。
   **步骤：** 使用计划中相同的零件、工序、机台/工作区域、日期或状态筛选。
   **预期结果：** WO/作业出现在预期筛选下，并映射到已记录的队列状态。
   **停止/联系条件：** 如果派工规则、队列就绪标签或队列状态不明确，请停止并联系生产主管。
...
| 班中 | 关注[队列系统](../10-production/queue-system.md)，响应生产主管联系处理的事项。 |
