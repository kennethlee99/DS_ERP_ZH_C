...
| 检查项 | 打开 | 安全结果 | 停止条件 |
|---|---|---|---|
| 身份 | [管理员设置清单](../01-workflows/admin-setup-checklist.md) | 登录用户、角色，以及如现场使用的工人身份已记录。 | 用户-工人关系仍是 `needs-decision`。 |
| 机台或工作区域 | [队列系统](../10-production/queue-system.md) | 所选机台、产线或工作区域与主管分派一致。 | 队列筛选或工作区域不明确。 |
| 队列行 | [操作员下一作业流程](../01-workflows/operator-run-next-job.md) | WO/作业、零件、工序和状态与分派一致。 | 派工规则或行优先级尚未确认。 |
| 开始操作 | [队列系统](../10-production/queue-system.md) | 行打开/开始控制有标签，或已由主管确认。 | 操作是不带标签的图标或隐藏控制。 |
| 报工页面 | 指定生产报工页面 | 页面、必填字段、保存操作和确认信号已知。 | 报工路径仍是 `needs-decision`。 |
| 交接证据 | [队列系统](../10-production/queue-system.md)或[工单](../10-production/production-orders.md) | 刷新后的状态、已保存记录或主管确认能证明变化。 | 队列行消失是唯一完成信号。 |
