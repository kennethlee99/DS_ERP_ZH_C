...
根据现场部署，日常执行可能在专用终端/看板流程中完成；本 Web 手册记录对应的工单、队列、
手动任务与质量录入。
...
4. 如需零件、数量、交期、工序或工单家族信息，打开[工单](../10-production/production-orders.md)。
5. 开工前确认是否有需要执行的检验指示。
...
| 问题 | 可能原因 | 下一步 |
|---|---|---|
| 看不到分派给我的工单 | 工单未发布、队列过滤不同、用户/机台分配错误 | 请[生产主管](production-supervisor.md)或计划员检查队列与工单状态 |
...
[手动任务](../10-production/manual-tasks.md)截图展示任务定义页面。已分派的手动工作仍需要在演示使用的队列或报工页面中确认。
